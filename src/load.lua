...
local function areHeightTilesEqual(a, b)
    ...
    return a.height == b.height and ax == bx and ay == by and aw == bw and ah == bh
end
...
function load.createHeightTextures()
    ...
    HEIGHT_MAPPED_SHADER:send("lensColor", Game.constants.lensColor)
...
function load.loadAudioFile(file, name, info)
    ...
    local ok, source = pcall(love.audio.newSource, file,
        ((info and info.size or 1) < (100 * 1024)) and "static" or "stream")

...
function PhysicsSystem.new()
    return setmetatable({}, PhysicsSystem)
end

function PhysicsSystem:enter(game)
...
function PhysicsSystem:findFloorAndCeiling(entity)
    ...
    for _, other in ipairs(self:getAllOverlappingOfType(entity.physics.heightSensor)) do
        if (entity.velocity or not other.velocity) and FruitSystem.shouldEntitiesContact(entity, other) then

...
function FruitSystem:update(framePart, dt, game)
    for _, entity in game:iterEntities() do
        self:updateEntity(framePart, dt, game, entity)
    end
end

function FruitSystem:updateEntity(framePart, dt, game, entity)
    -- Fruit stack haver
    if entity.fruitStack then
        ...
        for _, other in ipairs(game:getAllOverlappingOfType(entity.fruitStack.sensor)) do
            if other.fruit then
    ...
    if entity.picnic then
        ...
        for _, otherEntity in ipairs(game:getAllOverlappingOfType(entity.picnic.sensor)) do
    ...
    if entity.fruit then
        if entity.fruit.animFrames then
            ...
            if stackRootEntity.fruitStack then
                for _, otherEntity in ipairs(game:getAllOverlappingOfType(
                    entity.physics.heightSensor,
                    HEIGHT_SENSOR,
                    entity.pos))
                do
                    if FruitSystem.shouldEntitiesContact(entity, otherEntity) then
                        self:fruitDrop(entity, stackRootEntity, prevX, prevY, prevZ)
                    end
                end
            end
        else
...
function FruitSystem:fruitDrop(entity, stackRootEntity, prevX, prevY, prevZ)
    entity.pos.x = prevX
    entity.pos.y = prevY
    entity.pos.z = prevZ
